...
def load(model_path):
    net = PAN(in_nc=3, out_nc=3, nf=40, unf=24, nb=16, scale=2)
    load_network(model_path, net)
    return net
...
net = load('PANx2_DF2K.pth')
net.eval()
img = load_img('Lenna_(test_image).png') # BGR
img = img[:, :, [2, 1, 0]] # to RGB
img = torch.from_numpy(np.expand_dims( # to NCHW
    np.ascontiguousarray(np.transpose(img, (2, 0, 1))), axis=0)).float()
device = 'cpu'
img = img.to(device)
net = net.to(device)
with torch.no_grad():
...
out = out.detach()[0].float().cpu().numpy()
out = np.transpose(out, (1, 2, 0))
print(out.shape)
Image.fromarray(np.uint8(255.0 * out)).save('tmp.png')
